...
export default {
  ...
  async asyncData({ params, app }) {
    try {
      let userResponse = await app.$axios.$get(
        `v2/admin/users/${encodeURI(params.id)}`
      )
      let regionResponse = await app.$axios.$get('v2/admin/regions/all')
  ...
  methods: {
    ...
    async updatePersonalInfo() {
      ...
      try {
        await this.$axios
          .patch(`v2/admin/users/personal-info/${this.user.id}`, {
            name: this.form.name,
    ...
    async changePassword() {
      ...
      try {
        await this.$axios
          .patch(`v2/admin/users/change-password/${this.user.id}`, {
