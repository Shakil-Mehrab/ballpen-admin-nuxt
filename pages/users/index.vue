  ...
  <div class="flex">
    <div class="flex flex-col flex-1 min-w-0 overflow-hidden">
      <div class="relative z-0 flex flex-1 overflow-hidden">
        <main
          class="relative z-0 flex-1 bg-white focus:outline-none xl:order-last"
        >
          <nav
                ...
                <div class="-mt-12 sm:-mt-16 sm:flex sm:items-end sm:space-x-5">
                  ...
                  <div class="flex" v-else>
                    <img
                      ...
                      v-lazy="$auth.user.profile_photo_path"
                      :alt="$auth.user.name"
                    />
                  </div>
                  ...
                  >
                    ...
                    >
                      <nuxt-link
                        :to="link(user ? user.id : $auth.user.id)"
                        exact
                        type="button"
              ...
              <dl class="grid grid-cols-1 gap-x-4 gap-y-8 sm:grid-cols-2">
                <div class="sm:col-span-1">
                  ...
                  <dd class="mt-1 text-sm text-gray-900">
                    {{ user ? user.phone_number : $auth.user.phone_number }}
                  </dd>
                </div>
                ...
                <div class="sm:col-span-1">
                  ...
                  <dd class="mt-1 text-sm text-gray-900">Default</dd>
                </div>
                ...
                <div class="sm:col-span-2">
                  ...
                  <dd class="mt-1 space-y-5 text-sm text-gray-900 max-w-prose">
                    <p>{{ user ? user.bio : $auth.user.bio }}</p>
                  </dd>
                </div>
              </dl>
            </div>
...
export default {
  ...
  async asyncData({ app }) {
    let usersResponse = await app.$axios.$get('v2/admin/users')
    return {
  ...
  methods: {
    ...
    async showUser(id) {
      if (id) {
        try {
          await this.$axios.$get(`v2/admin/users/${id}`).then((response) => {
