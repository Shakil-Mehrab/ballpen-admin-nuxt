              ...
              <div class="w-full px-2">
                <div class="w-full py-4">
                  ...
                  <div v-else>
                    <img
                      :src="
                        article.thumbnails ? article.thumbnails.original : ''
                      "
                      alt="img"
            ...
            <div class="flex flex-wrap">
              <div class="w-full px-2">
                ...
                <div class="w-full py-4">
                  ...
                  <div v-else>
                    <img
                      :src="article.meta ? article.meta.image : ''"
                      alt="img"
                      class="w-full border rounded-md h-52"
                    />
                  </div>
...
export default {
  ...
  async asyncData({ params, app, error }) {
    try {
      let articleResponse = await app.$axios.$get(
        `v2/admin/articles/${encodeURI(params.slug)}`
      )
      ...
      return {
        ...
        form: {
          title: article.title,
          meta_title: article.meta ? article.meta.title : null,
          slug: article.slug,
          kicker: article.kicker,
          reporter: article.reporter,
          teaser: article.teaser,
          meta_description: article.meta ? article.meta.description : null,
          content: article.content,
          status: article.status,
          thumbnail: article.thumbnails ? article.thumbnails.id : null,
          meta_image: article.meta
            ? article.meta.image_id
            : article.thumbnails
            ? article.thumbnails.id
            : null,
  ...
  methods: {
    ...
    async articleUpdate() {
      ...
      try {
        await this.$axios
          .patch(`v2/admin/articles/${this.article.slug}`, this.form)
          .then(({ data }) => {
