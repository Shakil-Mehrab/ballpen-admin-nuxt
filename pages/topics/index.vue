      ...
      <div class="w-full md:flex md:space-x-3 lg:space-x-8">
        <div class="w-full md:w-4/12">
          <div class="p-2 border-2 border-dashed rounded-md sm:p-6 lg:p-8">
            <h1 class="text-xl font-semibold text-red-500">Create New Topic</h1>
...
export default {
  ...
  async asyncData({ app }) {
    let topicsResponse = await app.$axios.$get('v2/admin/topics?per-page=7')
    return {
  ...
  methods: {
    ...
    async deleteTopic(topicSlug) {
      try {
        await this.$axios
          .delete(`v2/admin/topics/${topicSlug}`)
          .then(({ data }) => {
            this.getTopics()
    ...
    async createTopic() {
      ...
      try {
        await this.$axios
          .post(`v2/admin/topics`, this.form)
          .then(({ data }) => {
    ...
    async getTopics(query = this.$route.query) {
      try {
        await this.$axios
          .$get('v2/admin/topics?per-page=7', {
            params: {
              page: query.page,
              ...query,
            },
